...
planilha = pd.read_excel("elasa28.xlsx")
...
for linha in planilha.index:
    ...
    try:
        ...
        print(f"Erro ao converter valor para nota {nota}: {valor_texto}")
        continue

    resultado_pesquisa = pesquisa(nota, tokens["access_token"])
    ...
    if resposta_baixa.status_code == 200:
        print(f"✓ Nota {nota} baixada com sucesso!")
    else:
        print(f"✗ Erro ao baixar nota {nota}: {resposta_baixa.status_code}")
